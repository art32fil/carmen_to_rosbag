...
#  ODOM| x | y |theta| tv| rv|accel|ipc_timestamp|ipc_hostname|logger_timestamp
	...
	TS_odom_robot_msg.header.stamp          = rospy.Time( float(words[6]) )
	TS_odom_robot_msg.header.frame_id       = laser_tf_parrent_frame
	TS_odom_robot_msg.child_frame_id        = laser_tf_child_frame
	TS_odom_robot_msg.transform.translation = Point(float(words[0]), float(words[1]), 0.0)
	quaternion = tf.transformations.quaternion_from_euler(0.0, 0.0, float(words[2]))
